...
exports.crearPeticion = async (req, res) => {
  const { id_usuario, tipo, titulo, mensaje, productora, fecha_estreno } = req.body;
  ...
  try {
    const [result] = await db.query(
      `INSERT INTO peticion (id_usuario, tipo, titulo, mensaje, productora, fecha_estreno)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id_usuario, tipo, titulo, mensaje, productora, fecha_estreno || null]
    );

    res.status(201).json({ message: 'Petición creada', id: result.insertId });
  } catch (err) {
